...
function drawGraph(r) {
    ...
    context.fillRect(center, center, r/2 * l, r * l);
...
function provideInteractiveGraphics() {
    ...
    canvas.addEventListener("click", function (e) {
        ...
        let spinnerWidget = PF('x_value');
        spinnerWidget.setValue(x);
        document.getElementById('form:y').value = y;
        document.getElementById("form:submit-button").click();
    });
}

document.getElementById('form:r').addEventListener("click", () => {
...
function addDot() {
    ...
    let hit = ((x >= -r) &&(y <= r/2) && (x <= 0) && (y >= 0) && (1.3*y - x/2<= r/2))
        || ((x >= 0) && (x <= r/2) && (y <= 0) && (y >= -r))
